...
function ClipboardPreviewDetail(props: { clip?: { text?: string; file?: string; html?: string }, onRefresh: () => void }) {
  ...
  return <Detail markdown={md} actions={<ActionPanel><Action title="Refresh" onAction={props.onRefresh} />{c?.text ? <Action.CopyToClipboard title="Copy Text" content={c.text} /> : null}{c?.file ? <Action.CopyToClipboard title="Copy File Path" content={c.file} /> : null}</ActionPanel>} />;
}
